...
class KeyDB:
  ...
  def save_generated_transaction(self, tx, cursor):
    ...
    '''
    for o in tx.outputs:
      index = self.encrypt_deterministic(o.serialized_index)
      pubkey = o.address.serialized_pubkey
      pubkey_ = self.encrypt(pubkey)
      taddress = self.encrypt(o.address.to_text().encode())
      value = self.encrypt_int(o.value)
      lock_height = self.encrypt_int(o.lock_height)
      ser_bl = self.encrypt(o.blinding_key.private_key)
      ser_apc = self.encrypt(o.serialized_apc)
      confirmed = 0
      now =  int(time.time())
      cursor.execute("""
